...
dept_employees1(Dept, [I|J]) -> 
    if I#employee.dept == Dept -> 
        [I|dept_employees1(Dept, J)]; 
        true -> dept_employees1(Dept, J) end.
...
dept_employees2(Dept, Employees) ->
  lists:filter(fun
    (#employee{dept = Department1}) 
    when Department1 == Dept -> true;
    (_) -> false
  end, 
  Employees).
...
dept_employees3(Dept,Employees)->
	[Em || Em<-Employees , Em#employee.dept == Dept].
...
delete_employee(Name,Employees)->
	[Em || Em<-Employees , Em#employee.name =/= Name].
...
upsert_employee(E, []) -> [E];
upsert_employee(E, [Em1|Tail]) 
when Em1#employee.name == E#employee.name -> [E|Tail];
upsert_employee(E, [Em1|Tail]) -> [Em1|upsert_employee(E,Tail)].
...
find_employees(Preds, Employees) ->
    [Em || Em <- Employees, lists:all(fun(Arg) -> Arg(Em) end,Preds)].
...
employees_req({read, Name}, Employees) ->
  case find_employees([employee_has_name(Name)], Employees) of
    [OPResult] -> {ok, OPResult, Employees};
    _ -> {err, "NON OR MORE THAN 1 WITH Similar Name", Employees}
  end;
...
employees_req_with_sort({sort}, Employees) ->
  INPUTS = [Input || #employee{name = Input} <- Employees],
  {_, SortedByName} = lists:unzip(lists:sort(lists:zip(INPUTS, Employees))),
...
employees_server(Fn, Employees) ->
  receive
    ...
    {Pid, Req} -> {Status, Opresult, Employees1} = Fn(Req, Employees), Pid ! {self(), {Status, Opresult}},
employees_server(Fn, Employees1)
  end.

% stop previously started server with registered ID emps.
% should return {ok, stopped}.
stop_employees_server() -> whereis(emps) ! {stop},erlang:unregister(emps),{ok, stopped}.
...
employees_client(Req) ->
  emps ! {self(), Req},
  Server = whereis(emps),
  receive {Server, {Status, Opresult}} ->
    {Status, Opresult}
  end.
