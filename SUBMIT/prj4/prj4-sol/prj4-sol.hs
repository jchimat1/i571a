...
comprDeptEmployees i department  = [j | j <- i, emplDept j == department]
...
employeesSalarySum employees = foldl (+) 0 ([emplSalary s | s<-employees])
...
quadraticRoots a b c = (root_first, root_second)
                        where
                          descriminant = b * b - (4 * a * c)
                          root_first = (- b / (2 * a)) + sqrt descriminant / (2 * a)
                          root_second = (- b / (2 * a)) - sqrt descriminant / (2 * a)
...
expn :: (Floating n) => n -> [n]
expnCalculate :: (Fractional n) => n -> n -> n -> n -> [n]

expn x = expnCalculate x 1.0 1.0 1
expnCalculate x numerator denominator fr = value:expnCalculate x new_numerator new_denominator new_fr
                        where
                            value = numerator/denominator
                            new_numerator = numerator * x
                            new_denominator = denominator * fr
                            new_fr = fr + 1
...
charIndexes input_string input_char = [ b | (a, b) <- zip input_string [0..], a == input_char ]
...
foldTree treeFn leafFn t = case t of 
    Leaf node -> leafFn node
    Tree leftSubTree node rightSubTree -> treeFn (foldTree treeFn leafFn leftSubTree) node (foldTree treeFn leafFn rightSubTree)
...
flattenTree t = foldTree (\leftSubTree value rightSubTree -> leftSubTree ++ [value] ++ rightSubTree) (\value -> [value]) t
...
catenateTreeLists (input_tree) =  [ output_list | tree_inputlist <- flattenTree input_tree, output_list <- tree_inputlist]
