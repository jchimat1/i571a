...
list_access(Indexes, List, Z) :- list_access(Indexes, 0, List, Z).
list_access(Indexes,num,[_|List],Z) :- num1 is num + 1, list_access(Indexes,num1,List,Z).
list_access(Indexes,_,List,[]) :- Indexes = [nil], List = [].
list_access([num|Indexes],num,[H|List],[H|Z]) :- num1 is num + 1, list_access(Indexes,num1,List,Z).
...
re_match(Re, List) :- (Re = List -> true; false).
...
clausal_form(PrologRules, Form) :- clausal_form(PrologRules, Form).
clausal_form(X, Z) :- parent(X, Y), ancestor(Y, Z).
